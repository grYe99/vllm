...
/// Label pairs for schema-driven KV connector metrics (`model_name` / `engine`
/// plus optional schema `const_labels`).
...
pub struct Metrics {
    registry: Registry,
    /// Families registered at runtime from third-party connector schemas.
...
impl Metrics {
    ...
    /// Mutate the dynamic (schema-driven) registry.
    ///
